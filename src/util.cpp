...
const char * const BITCOIN_CONF_FILENAME = "coin2fly.conf";
const char * const BITCOIN_PID_FILENAME = "coin2flyd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "coin2fly" is a composite category enabling all Coin2Fly-related debug output
            if(ptrCategory->count(string("coin2fly"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Coin2FlyCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Coin2FlyCore
    // Mac: ~/Library/Application Support/Coin2FlyCore
    // Unix: ~/.coin2flycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Coin2FlyCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Coin2FlyCore";
#else
    // Unix
    return pathRet / ".coin2flycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty coin2fly.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override coin2fly.conf
