...
*Setup instructions for a Gitian build of Coin2Fly Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coin2Fly
...
- [Building Coin2Fly Core](#building-coin2fly-core)
...
Clone the git repositories for Coin2Fly Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coin2fly/coin2flycore
```
...
in the Coin2Fly Core repository under 'Fetch and build inputs' to install sources which require
...
To build Coin2Fly Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Coin2Fly Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coin2fly/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/coin2fly/coin2flycore
...
For example:
```bash
URL=https://github.com/crowning-/coin2fly.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit coin2fly=${COMMIT} --url coin2fly=${URL} ../coin2fly/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coin2fly=${COMMIT} --url coin2fly=${URL} ../coin2fly/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coin2fly=${COMMIT} --url coin2fly=${URL} ../coin2fly/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the coin2fly git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../coin2fly/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/coin2fly.git
SIGPATH=/some/root/path/coin2fly-detached-sigs.git

./bin/gbuild --url coin2fly=${BTCPATH},signature=${SIGPATH} ../coin2fly/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coin2fly-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coin2fly-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/coin2fly-osx-build.assert
```
...
[coin2fly/gitian.sigs](https://github.com/coin2fly/gitian.sigs/) repository, or if that's not possible to create a pull
request.
